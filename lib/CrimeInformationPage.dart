...
class CrimeInformationPage extends StatefulWidget {
  final String incidentId;

  CrimeInformationPage({required this.incidentId});

  @override
  _CrimeInformationPageState createState() => _CrimeInformationPageState();
}

class _CrimeInformationPageState extends State<CrimeInformationPage> {
    ...
    controller.initialize().then((_) {
      ...
      controller.addListener(() {
        if (controller.value.position >= controller.value.duration) {
          Navigator.of(context).pop(); // Close the dialog when the video ends
          controller.dispose();
        }
      });
      showDialog(
      ...
      body: FutureBuilder<DocumentSnapshot>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return Center(child: Text('Error loading crime details'));
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      SizedBox(width: 8),
                      Text(
                        'Around ${DateFormat('dd MMM yyyy').format((data['crimeDateTime'] as Timestamp).toDate())}',
                        style: TextStyle(fontSize: 16),
                      ),
                    ],
                  ),
                  ...
                  Row(
                    children: [
                      ...
                      SizedBox(width: 8),
                      Text(
                        'At ${TimeOfDay.fromDateTime((data['crimeDateTime'] as Timestamp).toDate()).format(context)}',
                        style: TextStyle(fontSize: 16),
                      ),
                    ],
                  ),
